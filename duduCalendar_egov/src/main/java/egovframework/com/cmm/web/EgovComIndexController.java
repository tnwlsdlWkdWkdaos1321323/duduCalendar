...
public class EgovComIndexController implements ApplicationContextAware, InitializingBean {
	...
	public String setContent(ModelMap model) throws Exception {
		...
		if ( loginVO != null ) {
			...
			LOGGER.debug("EgovComIndexController ===>>> loginVO.getOfcpsNm() = " + loginVO.getOfcpsNm()); 
			LOGGER.debug("EgovComIndexController ===>>> loginVO.getOrgnztNm() = " + loginVO.getOrgnztNm()); 
			// 비밀번호 변경후 경과한 일수
