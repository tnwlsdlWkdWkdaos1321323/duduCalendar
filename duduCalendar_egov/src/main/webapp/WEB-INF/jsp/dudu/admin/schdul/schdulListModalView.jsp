			...
			<tbody>
				<c:forEach var="workList" items="${workList}" varStatus="status">
				    <tr class="lightGrayLine">
				        ...
				        <td onclick="barUserModal('${workList.schdulId}', '${workList.schdulNm}','${workList.schdulCn}','${workList.projectId}', '${workList.schdulBgnde}', '${workList.schdulEndde}')"><c:out value="${workList.schdulNm}" /></td><!-- 일정 제목 -->
				        <td><c:out value="${workList.schdulChargerName}"/></td><!-- 작성자 -->
				        <td><c:out value="${workList.schdulDeptId}" /></td><!-- 소속팀 -->
				    </tr>
