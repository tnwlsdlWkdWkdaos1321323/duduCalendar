...
<HEAD>
	...
	function delProject() {
	    ...
	    form.action = '/dudu/admin/projectDeleteActor.do';
	    form.submit();
	}
	
	// 프로젝트 목록 페이지로 이동
	function porjectList() {
		location.href = '/dudu/admin/projectList.do';
	}
	...
	function userModal(SCHDUL_ID, SCHDUL_NM, SCHDUL_CN, PROJECT_ID, SCHDUL_BGNDE, SCHDUL_ENDDE) {
	    ...
	    $.ajax({
	        url: '/dudu/admin/barSchdulDetail.do',
	        type: 'post',
	        data: {
	            "schdulId": SCHDUL_ID,
	            "schdulNm": SCHDUL_NM,
	...
	// bar 모달 -> 사원 일정 상세보기
	function barUserModal(SCHDUL_ID, SCHDUL_NM, SCHDUL_CN, PROJECT_ID, SCHDUL_BGNDE, SCHDUL_ENDDE) {
	    console.log(SCHDUL_ID);
	    $.ajax({
	        url: '/dudu/admin/barSchdulDetail.do',
	        type: 'post',
	        ...
	        success: function(data) {
	            var barUserModalView = document.getElementById('barUserModalView');
	            var barModalView = document.getElementById('barModalView');
	            barUserModalView.innerHTML = data;
	            $('#barUserModalView').css('display' , 'block');
	            $('#barModalView').css('display', 'none');
	        },
	        error: function(error) {
	            alert("새로고침 후 다시시도 부탁드립니다.");
	...
	function closeMoUserModal() {
		var modal = document.getElementById('barUserModalView');
		var barModalView = document.getElementById('barModalView');
		modal.style.display = "none";
		barModalView.style.display = "block";
	}
...
<section class="contents">
	<div class="leftContent">
		<div style="display: none;" class="registBtn" onclick="registModal()">프로젝트 등록</div>
		<!-- 프로젝트 등록 모달 시작 -->
			...
			<form id="listForm" name="listForm" action="/dudu/calendar.do" method="post">
				<div class="listContent">
					<c:forEach var="result" items="${resultList}" varStatus="status" >
					    <span class="listColor" style="background-color: #606868;">
					    </span>
...
<div class="modal fade" id="barUserModalView" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true" style="display: none;"></div>
</BODY>
</HTML>
