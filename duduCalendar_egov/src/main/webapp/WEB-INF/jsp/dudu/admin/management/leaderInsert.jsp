...
<c:set var="pageTitle">팀장 등록</c:set>
<!DOCTYPE html>
<html>
<head>
<title>팀장 등록</title>
...
<link type="text/css" rel="stylesheet" href="/resources/css/admin/leaderInsert.css">
...
<!-- content start -->
<form:form commandName="duduUserJoinVO" action="${pageContext.request.contextPath}/uss/umt/leaderInsert.do" name="duduUserJoinVO" method="post">

<div class="wTableFrm">
	<!-- 타이틀 -->
	<div class="logo-div">
		<a href="/uat/uia/duduLoginUsr.do?mode=2">
			<img alt="팀장 로고" src="/resources/images/admin_logo.svg">
		</a>
	</div>
	<div class="wTableHead">팀장 등록</div>
	...
	<tbody>
		...
		<!-- 팀장 아이디 -->
		<c:set var="title">팀장 아이디</c:set>
		<tr class="join-tr">
			<td class="left">
				<div class="left-title-first">아이디</div>
				<form:input path="entrprsmberId" id="entrprsmberId" title="${title} ${inputTxt}" cssClass="txaIpUmt right-content-first" size="20" readonly="true" maxlength="20" placeholder="영문, 소문자, 숫자 사용 가능" />
		...
		<!-- 팀장 이름 -->
		<c:set var="title">팀장 이름</c:set>
		<tr>
			<td class="left">
				<div class="left-title">성명</div>
				<form:input path="cmpnyNm" title="${title} ${inputTxt}" cssClass="txaIpUmt name-content" size="50" maxlength="60" placeholder="본명을 입력해주세요" />
		...
		<tr>
			<td class="left">
				<div>
					...
					<form:password path="entrprsMberPassword" title="${title} ${inputTxt}" cssClass="txaIpUmt pw-content" size="50" maxlength="20" placeholder="문자, 숫자, 특수 문자 혼용, 8~10자리" />
		...
		<tr>
			<td class="left">
				...
				<input name="entrprsMberPassword2" id="entrprsMberPassword2" title="${title} ${inputTxt}" class="txaIpUmt pw1-content" type="password" size="50" maxlength="20" placeholder="비밀번호를 다시 입력하세요" />
			</td>
		</tr>
		<!-- 부서 -->
	...
	<div class="btn regBtn">
		<button type="button" class="s_submit" value="팀장 등록" title="팀장 등록" onclick="regBtn()">팀장 등록</button>
