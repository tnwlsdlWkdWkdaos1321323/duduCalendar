...
<div class="sort_area">
	<div class="select_group">
     	<select style="display: none;" name="schdulSe" class="select" id="schdulSe" onChange="fnEgovSchdulSe(document.forms[0].schdulSe.options[document.forms[0].schdulSe.selectedIndex].value);" title="일정구분선택">
	...
	<div class="date_view" style="left:45%;">
		<ul>
			<li style="display: none;"><a href="javascript:fnEgovSchdulTodate();" class="today"><spring:message code="comCopSmtSdm.Navi.Today"/></a></li><!-- 오늘 -->
...
for (int index = 1; index <= endDay; index++) {
	...
	if (listResult != null) {
	    for (int i = 0; i < listResult.size(); i++) {
	        ...
	        System.out.println("test ===>>> 27 : egovMap => " + egovMap);
	        System.out.println("!~~~@TEST@~~~~!===>>>27");
	
	        iBeginDate = Integer.parseInt(((String) egovMap.get("projectBgnde")).substring(0, 8));
	        iBeginEnd = Integer.parseInt(((String) egovMap.get("projectEndde")).substring(0, 8));
	...
	if (iDayPreviewCount > 4) {
	    out.println("<TD id='id_" + iUseDate + "' valign='top' align='left' height='" + (130 + ((iDayPreviewCount - 4) * 24)) + "px' nowrap onclick=\"fn_egov_regist_DeptSchdulManage('" + iUseDate + "')\">");
	} else {
	    out.println("<TD id='id_" + iUseDate + "' valign='top' align='left' height='130px' nowrap onclick=\"fn_egov_regist_DeptSchdulManage('" + iUseDate + "')\">");
	}

    out.println("<a href='javascript:void(0);'><font color='" + color + "'>" + index + "</font></a>");
    out.println("<BR>");
            ...
            if (iUseDate >= iBeginDate && iUseDate <= iBeginEnd) {
                ...
                if (iSchdulWeekFirest == 1 || (iSchdulWeekFirest == 0 && iSchdulCurrent == iSchdulCurrentTotal - 1)) { //첫 토요일 보정 처리
                    ...
                    if ((iSchdulCurrent + 1) < 6) {
                        iDotTextWidth = 175 * (iSchdulCurrent + 2);
                    } else {
                        iDotTextWidth = 175 * (7 - newLine);
                    }
                    ...
                    out.print("<div class='divWeekBar' style='margin-top:" + (iDayCount * 20) + "px;position:absolute;width:" + iDotTextWidth + "px;border:solid 0px;background-color:" + egovMap.get("schdulWeekColor") + "; cursor:pointer;' onclick=\"document.getElementById('linkWeek_" + (String) egovMap.get("projectId") + "').click()\">");
                    out.print("<a id='linkWeek_" + (String) egovMap.get("projectId") + "' href=\"JavaScript:fn_egov_detail_DeptSchdulManage('" + (String) egovMap.get("projectId") + "')\" class='linkWeek'>");
                    out.print((String) egovMap.get("projectNm"));
                    out.println("</a></div>");
                }
                //일별스케줄갯수
                iDayCount++;
