...
<div class="popup">
			...
			<li>
				<select name="searchCondition" title="<spring:message code="title.searchCondition" /> <spring:message code="input.cSelect" />">
					<option selected value=''>--<spring:message code="input.select" />--</option>
			   		...
			   		<option value='EMPLY_ID' <c:if test="${searchCondition == 'RESEARCHER_ID'}">selected</c:if>><spring:message code="comCopSmtSdm.empLyrList.name" /></option><!-- 아이디 -->
	...
	<tr>
		...
		<td class="left">${resultInfo.researcherId}</td>
		<td class="left">${resultInfo.userNm}</td>
	   	<td>
	   		<button class="btn_style3" onClick="javascript:fn_egov_open_Popup('${status.count}', '${resultInfo.esntlId}'); return false;" title="<spring:message code="button.select" /> <spring:message code="input.button" />"><spring:message code="button.select" /></button><!-- 선택 -->
	   		<input name="iptText_${status.count}" id="iptText_${status.count}" type="hidden" value="${resultInfo.researcherId}">
	   	</td>
	</tr>
	</c:forEach>
	</tbody>
